...
/// Repair the open segment if requested and return its last complete commit
/// without reading closed segments.
pub(crate) fn last_open_commit(
    path: &Path,
    repair: bool,
) -> Result<(Option<DurableCommit>, RecoveryReport)> {
    let events = path.join("events");
    let mut open = None;
    for entry in fs::read_dir(&events).map_err(|source| io_error(&events, source))? {
        let path = entry.map_err(|source| io_error(&events, source))?.path();
