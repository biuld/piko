//! Cross-process Session Trajectory queries (F-52 / D-69).
//!
...
fn history_survives_hostd_restart_without_journal_segments() {
    ...
    assert!(
        before.agents.iter().any(|agent| agent.work_count > 0),
    ...
    let root = after
        ...
        .expect("root agent after restart");
    host.send(Command::SessionHistoryAgentStreamGet {
        command_id: "stream-after".into(),
    ...
    match host.command_result("stream-after") {
        CommandResult::SessionHistoryAgentStreamPaged { page, .. } => {
    ...
    host.send(Command::SessionHistoryLaneGet {
        command_id: "lane-after".into(),
        session_id: session_id.clone(),
        agent_instance_id: root,
        expected_revision: after.revision,
    });
    match host.command_result("lane-after") {
        CommandResult::SessionHistoryLaneGot { summary, .. } => {
            assert_eq!(summary.session_id, session_id);
        }
        other => panic!("expected lane summary, got {other:?}"),
    }
}
