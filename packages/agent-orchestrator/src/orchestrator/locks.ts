...
import type { LockMode, LockState } from "piko-engine-protocol";
...
export function requestLock(
  ...
  emitToCtx(ctx, { type: "lock_requested", lockId, agentId, taskId, resource, mode });

  if (!lock.holderAgentId) {
    lock = { ...lock, holderAgentId: agentId, holderTaskId: taskId, mode };
    ctx.state.locks = { ...ctx.state.locks, [lockId]: lock };
    emitToCtx(ctx, { type: "lock_acquired", lockId, agentId, taskId, resource, mode });
    return true;
  }
...
export function releaseLock(
  ...
  emitToCtx(ctx, { type: "lock_released", lockId, agentId, taskId, resource });
