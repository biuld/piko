...
pub trait AgentRunRunner: Send + Sync {
    ...
    /// runtime extras (prompt staging, tool restriction). The durable facts
    /// stay on `input`; extras are never persisted.
    ...
    /// Interrupt the active root of an AgentInstance. Agent-addressed; does
    /// not require a host Turn.
