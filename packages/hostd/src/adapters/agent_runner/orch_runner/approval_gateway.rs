...
use crate::adapters::agent_runner::approval::ApprovalScope;
    ...
    async fn request_tool_approval(&self, request: ToolApprovalRequest) -> ToolApprovalDecision {
        ...
        if let Err(error) = self
            .commit_agent_work_fact(
                &session_id,
                piko_protocol::AgentDurableCommand::PendingActionRequested {
                    agent_instance_id: request.agent_instance_id.clone(),
                    root_input_id: request.root_input_id.clone(),
                    action: piko_protocol::PendingActionSummary {
                        action_id: approval_id.clone(),
                        kind: "approval".into(),
                        summary: Some(request.tool_name.clone()),
                    },
                    requested_at: now_ms(),
                },
            )
            .await
        {
            tracing::error!(%error, %approval_id, "failed to persist pending approval");
            return ToolApprovalDecision::Decline;
        }
        {
        ...
        let decision = match tokio::time::timeout(self.approval_timeout, rx).await {
            ...
            Err(_elapsed) => {
                ...
                if let Err(error) = self
                    ...
                    .await
                {
                    tracing::error!(%error, %approval_id, "failed to persist approval expiry");
                }
                self.observation_router
                    .publish(
                        &session_id,
