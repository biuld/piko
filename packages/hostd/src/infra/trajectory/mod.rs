...
use piko_comms::contracts::{TrajectoryLive, TrajectoryWrites};
...
use piko_protocol::{
    ...
    TRAJECTORY_EVENT_SYSTEM_NOTIFICATION, TRAJECTORY_EVENT_TOOL_CALL, TrajectoryLiveEvent,
    TrajectoryRecord,
};
    ...
    pub fn new(store: SessionStore, session_id: String) -> Self {
        ...
        tokio::spawn(async move {
            while let Some(write) = rx.recv().await {
                ...
                match result {
                    Ok(Ok(revision)) => {
                        let _ = live_writer.send(TrajectoryLiveEvent {
