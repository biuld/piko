...
pub fn snapshot_prompt_resources(
    ...
    if let Some(content) = environment_host {
        blocks.push(block(
            ...
            piko_protocol::InstructionAuthority::None,
            piko_protocol::ContentTrust::Trusted,
            piko_protocol::PromptSource::new("environment", "host"),
            content,
            piko_protocol::CacheScope::RunDynamic,
        ));
    }
    if let Some(content) = model_switch {
        blocks.push(block(
            "context.model-switch",
            piko_protocol::PromptBlockKind::Context,
            piko_protocol::InstructionAuthority::None,
            piko_protocol::ContentTrust::Trusted,
            piko_protocol::PromptSource::new("environment", "model-switch"),
            content,
            piko_protocol::CacheScope::RunDynamic,
        ));
    }
...
fn environment_host_content(
    environment: &super::environment::EnvironmentSnapshot,
) -> Option<String> {
    let mut lines = Vec::new();
    if let Some(os) = environment.os.as_deref() {
        lines.push(format!("os: {os}"));
    }
    if let Some(arch) = environment.arch.as_deref() {
        lines.push(format!("arch: {arch}"));
    }
    if let Some(shell) = environment.shell.as_deref() {
        lines.push(format!("shell: {shell}"));
    }
    if let Some(hostname) = environment.hostname.as_deref() {
        lines.push(format!("hostname: {hostname}"));
    }
    if let Some(timezone) = environment.timezone.as_deref() {
        lines.push(format!("timezone: {timezone}"));
    }
    if let Some(locale) = environment.locale.as_deref() {
        lines.push(format!("locale: {locale}"));
    }
    (!lines.is_empty()).then(|| lines.join("\n"))
}

fn model_switch_content(options: &PromptSnapshotOptions) -> Option<String> {
    let (previous, current) = (
        options.previous_model.as_deref()?,
    ...
    Some(format!(
        "<model_switch>\n{MODEL_SWITCH_PREAMBLE} \"{previous}\"; this run executes on \"{current}\". Treat prior context as generated by a different model.\n</model_switch>"
    ))
}
