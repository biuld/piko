...
impl HostApp {
    ...
    pub(crate) async fn ensure_agent_session_dir(
        &self,
        session_id: &str,
        cwd: &str,
    ...
    pub(crate) async fn submit_user_input(
        ...
        self.run_registered_input(command_id, input, tx).await
    }

    async fn run_registered_input(
        &self,
        command_id: String,
        mut input: piko_protocol::AgentInput,
        tx: &ClientEventSender,
    ) -> Result<(), ProtocolError> {
        let session_id = input.session_id.clone();
        let agent_instance_id = input.agent_instance_id.clone();
        let root_input_id = input.input_id.clone();
        let cwd = {
        ...
        let expanded_content = super::content::expand_templates(input.content.clone(), &templates);
        ...
        input.content = expanded_content;
        let runtime = AgentInputRuntime {
            prompt_resources: Some(prompt_resources),
            active_tool_names,
            root_input_id: None,
            message_id: Some(format!("msg_user_{}", uuid::Uuid::new_v4())),
        };
        let receipt = match runner.submit_agent_input(input, runtime).await {
            Ok(receipt) => receipt,
            Err(error) => {
                send_event(
                    tx,
