...
#[tokio::test]
async fn stream_pages_a_large_session_without_copying_full_bodies() {
    ...
    let overview = query.overview("s1").await.unwrap();
    let first = query
        .agent_stream("s1", agent, overview.revision, None, Some(20))
        .await
        .unwrap();
    assert_eq!(first.items.len(), 20);
    assert!(first.next_cursor.is_some());
    assert!(first.items.iter().all(|item| !item.summary.contains(&body)));

    let mut seen = first.items.len();
    let mut cursor = first.next_cursor.clone();
    while let Some(next) = cursor {
        let page = query
            .agent_stream("s1", agent, overview.revision, Some(&next), Some(20))
            .await
            .unwrap();
        seen += page.items.len();
    ...
    let input = first.items.first().unwrap();
...
async fn usage_correction_and_failed_work_survive_aligned_reads() {
    ...
    let page = query
        ...
        .await
        .unwrap();
    // Usage corrections and work reports are not stream rows; the stream stays
    // flat over inputs, messages, and steps.
    assert!(
        page.items
            .iter()
            .all(|item| !matches!(item.kind.0.as_str(), "usage" | "report"))
    );
    std::fs::rename(
