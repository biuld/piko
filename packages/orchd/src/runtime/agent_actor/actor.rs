...
impl AgentActor {
    pub fn new(spec: crate::domain::agents::spec::AgentSpec, deps: AgentActorDeps) -> Self {
        Self {
...
impl AgentActor {
    ...
    fn handle_set_model_config(&mut self, config: crate::domain::model::step::ModelConfig) {
    ...
    async fn handle_steer(
        ...
        if state.current_task_id.as_deref() == Some(&task_id)
            && state.status == AgentStatus::Running
        {
            state.steering_queue.push(
                crate::domain::tasks::steering::SteerMessage {
                    source_task_id,
                    source_agent_id,
                    message,
                },
    ...
    for message in &result.messages {
        match message {
            Message::Assistant {
                ...
                let message_id = runtime_assistant_message_id(task_id, &format!("step_{assistant_index}"));
                assistant_index += 1;
...
fn host_usage_from_message_usage(usage: &crate::domain::model::transcript::MessageUsage) -> HostUsage {
    HostUsage {
        input: usage.input,
        output: usage.output,
