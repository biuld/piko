...
use piko_protocol::execution::{CancelReason, ExecutionOutcome, ExecutionSnapshot, SteerExecutionRequest};
...
pub struct ArcTerminalReceiver {
    inner: std::sync::Arc<tokio::sync::Mutex<Option<oneshot::Receiver<ExecutionOutcome>>>>,
}

impl ArcTerminalReceiver {
    pub fn new(rx: oneshot::Receiver<ExecutionOutcome>) -> Self {
        Self {
            inner: std::sync::Arc::new(tokio::sync::Mutex::new(Some(rx))),
        }
    }

    pub async fn wait(self) -> Result<ExecutionOutcome, AgentApiError> {
