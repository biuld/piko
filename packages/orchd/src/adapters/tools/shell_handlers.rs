// ---- `bash` tool: one-shot sandboxed PTY execution ----
//
// Runs through the piko-sandbox PTY runner (F-08): the shell is the
// session/process-group leader, output is bounded, and timeout/cancellation
// escalate SIGTERM → SIGKILL to the whole group. The OS sandbox wraps the
// command when `os_sandbox` is set.
...
pub(super) fn shell_tool_def() -> ToolDef {
    ToolDef {
        name: "bash".into(),
        version: "1".into(),
...
pub(super) async fn execute_shell_tool(
    policy: &Policy,
    shell: &ShellSnapshot,
    os_sandbox: bool,
    call: &ToolCall,
    ...
    // Validate command against policy (fast-fail static allowlist; the OS
...
mod tests {
    ...
    async fn bash_reports_exit_code_and_signal() {
        ...
        let signalled = execute_shell_tool(
            &policy,
            &shell,
            false,
