...
- Paint uses **no body fill** (plane background shows through). Border is
  `prompt_border_active` when Chat owns editor focus, otherwise
  `prompt_border`. Pointer hover does not add a competing visual state.
