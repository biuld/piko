...
impl SettingsPanel {
    ...
    fn build_settings_tree() -> SettingsNode {
        let group = |title: &str, detail: &str, children: Vec<SettingsNode>| SettingsNode::Group {
            title: title.to_string(),
            detail: detail.to_string(),
            children,
        };

        let action = |title: &str, detail: &str, act: SettingsAction| SettingsNode::Action {
            title: title.to_string(),
            detail: detail.to_string(),
            action: act,
        };
        ...
        group(
            ...
            vec![
                group(
                    "Thinking Level",
                    "Select assistant reasoning and thinking budget",
                    vec![
                        action(
                            "off",
                            "Disable assistant thinking/reasoning",
                            SettingsAction::Thinking("off"),
                        ),
                        action(
                            "minimal",
                            "Minimal reasoning budget",
                            SettingsAction::Thinking("minimal"),
                        ),
                        action(
                            "low",
                            "Low reasoning budget",
                            SettingsAction::Thinking("low"),
                        ),
                        action(
                            "medium",
                            "Medium reasoning budget",
                            SettingsAction::Thinking("medium"),
                        ),
                        action(
                            "high",
                            "High reasoning budget",
                            SettingsAction::Thinking("high"),
                        ),
                        action(
                            "xhigh",
                            "Extra high reasoning budget (maximum)",
                            SettingsAction::Thinking("xhigh"),
                        ),
                    ],
                ),
                group(
                    "Thinking Blocks",
                    "Show or hide thinking content in future rendering",
                    vec![
                        action(
                            "Hide thinking blocks",
                            "Hide thinking content in future rendering where supported",
                            SettingsAction::HideThinking(true),
                        ),
                        action(
                            "Show thinking blocks",
                            "Show thinking content in future rendering where supported",
                ...
                group(
                    ...
                    "Configure compaction settings and reserve token sizes",
                    vec![
                        action(
                            "Enable compaction",
                        ...
                        group(
                            "Reserve Tokens",
                            "Set number of tokens to reserve for context window",
                            vec![
                                action(
                                    "Reserve 8k tokens",
                                    "Reserve 8,192 tokens for system context",
                                    SettingsAction::CompactionReserve(8192),
                                ),
                                action(
                                    "Reserve 16k tokens (Default)",
                                    "Reserve 16,384 tokens for system context",
                                    SettingsAction::CompactionReserve(16384),
                                ),
                                action(
                                    "Reserve 32k tokens",
                                    "Reserve 32,768 tokens for system context",
                                    SettingsAction::CompactionReserve(32768),
                                ),
                            ],
                        ),
                        group(
                            "Keep Recent Tokens",
                            "Set number of recent tokens to preserve in full detail",
                            vec![
                                action(
                                    "Keep 10k tokens",
                                    "Keep 10,000 recent tokens intact",
                                    SettingsAction::CompactionKeep(10000),
                                ),
                                action(
                                    "Keep 20k tokens (Default)",
                                    "Keep 20,000 recent tokens intact",
                                    SettingsAction::CompactionKeep(20000),
                                ),
                                action(
                                    "Keep 30k tokens",
                                    "Keep 30,000 recent tokens intact",
                                    SettingsAction::CompactionKeep(30000),
                                ),
                                action(
                                    "Keep 50k tokens",
                                    "Keep 50,000 recent tokens intact",
                                    SettingsAction::CompactionKeep(50000),
                                ),
                            ],
                        ),
                    ],
                ...
                group(
                    "API Retries",
                    "Enable/disable automatic API connection retries",
                    vec![
                        action(
                            "Enable retries",
                            "Enable automatic retries on LLM API failure",
                            SettingsAction::Retry(true),
                        ),
                        action(
                            "Disable retries",
                            "Disable automatic retries on LLM API failure",
                ...
                group(
                    "Tool Sandbox",
                    "Enable/disable sandboxed tool execution execution limits",
                    vec![
                        action(
                            "Enable sandbox",
                            "Enable filesystem & shell sandboxing rules",
                            SettingsAction::Sandbox(true),
                        ),
                        action(
                            "Disable sandbox",
                            "Disable filesystem & shell sandboxing rules",
                ...
                group(
                    "UI Theme",
                    "Set UI color theme preference",
                    vec![
                        action(
                            "Theme dark",
                            "Set TUI theme to dark theme",
                            SettingsAction::Theme("dark"),
                        ),
                        action(
                            "Theme light",
                            "Set TUI theme to light theme",
                ...
                group(
                    ...
                    "Set host transport preference",
                    vec![action(
                        "Transport stdio",
                        "Set host transport preference to stdio",
                        SettingsAction::Transport("stdio"),
                    )],
                ),
                group(
                    "Active Tools Mode",
                    "Configure active tools allowance",
                    vec![
                        action(
                            "Enable all tools",
                            "Reset and allow all discovered tools",
