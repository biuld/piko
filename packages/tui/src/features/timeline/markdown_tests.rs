...
fn fenced_code_block_renders_without_pseudo_border() {
    ...
    assert_eq!(
        rendered_text(&lines),
        vec![
            format!("1  first{}", " ".repeat(72)),
            format!("2  second{}", " ".repeat(71)),
        ]
    );
...
fn known_fenced_language_uses_syntax_highlighting() {
    ...
    assert_eq!(rendered_text(&lines).len(), 1);
    assert_eq!(
        rendered_text(&lines)[0].trim_end(),
        "1  fn main() { let value = \"text\"; }"
    );
    assert_eq!(rendered_text(&lines)[0].chars().count(), 80);
...
fn unknown_fenced_language_falls_back_to_plain_text() {
    ...
    assert_eq!(rendered_text(&lines), vec!["1  hello world      "]);
    assert!(
        lines
            .iter()
            .flat_map(|line| line.spans.iter())
            .all(|span| span.style.bg == Some(Theme::dark().md_code_bg))
    );
}
...
| 编辑器 | Vim |
";
    let lines = parse_markdown(md, &Theme::dark(), 80);
    let plain = rendered_text(&lines).join("\n");
