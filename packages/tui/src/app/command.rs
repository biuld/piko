...
pub enum SlashAction {
    ...
    Delete,
    Login(Option<String>),
    LoginDevice(Option<String>),
    CancelLogin(Option<String>),
...
const HOST_SLASH_TABLE: &[(&str, &str)] = &[
    ...
    ("/login", "auth.login"),
    ("/login-device", "auth.login-device"),
    ("/login-cancel", "auth.cancel-login"),
    ("/logout", "auth.logout"),
...
pub fn action_for_host_command(id: &str, args: HostCommandArgs) -> Option<Action> {
    Some(match id {
        ...
        "auth.login-device" => SlashAction::LoginDevice(args.provider).into(),
        "auth.cancel-login" => SlashAction::CancelLogin(args.provider).into(),
