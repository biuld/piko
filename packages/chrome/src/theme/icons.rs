//! Vendored Lucide-compatible icons for Islands chrome.
//!
//! SVG bytes: [`crate::ChromeAssets`]. Path strings must stay in sync with
//! `packages/chrome/assets/icons/`.
...
/// Typed subset of Lucide icons used by this chrome kit.
...
pub enum ChromeIcon {
    ...
    Settings,
    /// TitleBar left dock toggle — hollow when the column is closed.
    PanelLeft,
    /// TitleBar left dock toggle — filled pane when the column is docked.
    PanelLeftFilled,
    /// TitleBar right dock toggle — hollow when the column is closed.
    PanelRight,
    /// TitleBar right dock toggle — filled pane when the column is docked.
    PanelRightFilled,
    Pin,
}

impl IconNamed for ChromeIcon {
...
pub fn icon(name: ChromeIcon, size: IconSize, color: impl Into<Hsla>) -> Icon {
    Icon::new(name)
...
pub fn row_leading(name: ChromeIcon, color: impl Into<Hsla>) -> gpui::AnyElement {
    div()
        .w(px(16.))
        .flex_shrink_0()
        .flex()
...
pub fn rotating_gear(color: impl Into<Hsla>, animate: bool) -> gpui::AnyElement {
    ...
    let mark: gpui::AnyElement = if animate {
        gpui_component::spinner::Spinner::new()
            .icon(Icon::new(ChromeIcon::Settings))
            .with_size(Size::Size(IconSize::Meta.pixels()))
            .color(color)
            .into_any_element()
    } else {
        icon(ChromeIcon::Settings, IconSize::Meta, color).into_any_element()
    };
    div()
        .w(px(16.))
        .flex_shrink_0()
...
pub fn disclosure(expanded: bool, color: impl Into<Hsla>) -> Icon {
    icon(
        if expanded {
            ChromeIcon::ChevronDown
        } else {
            ChromeIcon::ChevronRight
        },
        IconSize::Meta,
        color,
    )
...
pub fn panel_toggle_icon(side: PanelSide, docked: bool, color: impl Into<Hsla>) -> Icon {
    let name = match (side, docked) {
        (PanelSide::Left, false) => ChromeIcon::PanelLeft,
        (PanelSide::Left, true) => ChromeIcon::PanelLeftFilled,
        (PanelSide::Right, false) => ChromeIcon::PanelRight,
        (PanelSide::Right, true) => ChromeIcon::PanelRightFilled,
    };
    icon(name, IconSize::Label, color)
}

/// Which outer dock column a panel toggle controls.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum PanelSide {
    Left,
    Right,
...
pub fn placeholder_icon(name: ChromeIcon, color: impl Into<Hsla>) -> impl IntoElement {
...
mod tests {
    ...
    #[test]
    fn paths_are_stable() {
        assert_eq!(ChromeIcon::Plus.path().as_ref(), "icons/plus.svg");
        assert_eq!(ChromeIcon::Bot.path().as_ref(), "icons/bot.svg");
        assert_eq!(ChromeIcon::User.path().as_ref(), "icons/user.svg");
        assert_eq!(ChromeIcon::Settings.path().as_ref(), "icons/settings.svg");
        assert_eq!(ChromeIcon::Folder.path().as_ref(), "icons/folder.svg");
        assert_eq!(ChromeIcon::Network.path().as_ref(), "icons/network.svg");
        assert_eq!(
            ChromeIcon::PanelLeft.path().as_ref(),
            "icons/panel-left.svg"
        );
        assert_eq!(
            ChromeIcon::PanelLeftFilled.path().as_ref(),
            "icons/panel-left-filled.svg"
        );
        assert_eq!(
            ChromeIcon::PanelRightFilled.path().as_ref(),
            "icons/panel-right-filled.svg"
        );
        assert_eq!(ChromeIcon::Pin.path().as_ref(), "icons/pin.svg");
