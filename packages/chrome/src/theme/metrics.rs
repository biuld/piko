//! Shared density and type-scale numbers for Islands chrome.
//!
//! Islands must not hard-code px sizes for text; go through
//! [`crate::theme::typography`] (or [`crate::theme::icons`] for icon boxes).
//! Spacing and layout constants are read via [`metrics`].

use gpui::{Pixels, px};

/// One compact scale shared by every first-class chrome surface.
