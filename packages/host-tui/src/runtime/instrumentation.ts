// ============================================================================
// Dev-only TUI instrumentation — gated by PIKO_TUI_TRACE env var.
...
export function traceDispatch(eventType: string): void {
  ...
  if (now - lastDispatchTime >= 1000) {
    ...
    if (dispatchRate > 30) {
      console.error(`[tui trace] high dispatch rate: ${dispatchRate}/s (type: ${eventType})`);
    }
  }
...
export function traceTimelineHeight(height: number): void {
  ...
  if (lastTimelineHeight !== undefined && lastTimelineHeight !== height) {
    console.error(
      `[tui trace] timeline scrollbox height changed: ${lastTimelineHeight} -> ${height}`,
    );
  }
