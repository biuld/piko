...
export function doUpdateHeader(app: HeaderFooterDeps): void {
  ...
  app.headerBox.addChild(new Text(t.fg("accent", ` piko  ${app.currentModel.provider}/${app.currentModel.id}  session ${name}  ${app.transcript.length} msgs `), 1, 0));
  app.headerBox.addChild(new DynamicBorder((s: string) => t.fg("border", s)));
}

export function doUpdateFooter(app: HeaderFooterDeps): void {
  const statuses = [...app.statusLine.getEntries()];
  const hints = app.running ? getContextHints("streaming") : app.activeOverlay ? getContextHints("overlay") : getContextHints("normal");
  app.footerComponent.update({
    model: app.currentModel, sessionName: app.sessionName, messageCount: app.transcript.length, cwd: app.host.cwd,
    ...
    contextPercent: (app.currentModel as { contextWindow?: number }).contextWindow
      ? getContextPercent(app.cumulativeInput, (app.currentModel as { contextWindow?: number }).contextWindow!) : undefined,
