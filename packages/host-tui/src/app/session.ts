...
export async function doResume(app: SessionDeps): Promise<void> {
  const loaded = await app.host.loadMessages();
  if (loaded.length === 0) { app.chatView.addMessage("system", `Session ${app.host.sessionId} not found or empty`); app.chatView.rebuildChat(); app.tui.requestRender(); return; }
  ...
  app.chatView.addMessage("system", `Resumed session ${app.host.sessionId} (${loaded.length} messages)`);
  app.chatView.rebuildChat(); app.tui.requestRender();
}
...
export async function doClone(app: SessionDeps): Promise<void> {
  if (!app.host.isSessionPersisted()) { app.chatView.addMessage("system", "Clone requires a saved session"); app.chatView.rebuildChat(); app.tui.requestRender(); return; }
...
export async function doFork(app: SessionDeps, entryId: string): Promise<void> {
  if (!app.host.isSessionPersisted()) { app.chatView.addMessage("system", "Fork requires a saved session"); app.chatView.rebuildChat(); app.tui.requestRender(); return; }
