...
export function doRunStream(
  app: SubmitDeps,
  stream: ReturnType<BaseApp["host"]["streamPrompt"]>,
): void {
  ...
  void (async () => {
    for await (const e of stream) {
      if (e.type === "message_delta") {
        if (hasAssistant) {
          app.chatView.updateLastAssistant((e as any).delta);
        } else {
          hasAssistant = true;
          app.chatView.addMessage("assistant", (e as any).delta);
        }
        app.chatView.rebuildChat();
        app.tui.requestRender();
      } else if (e.type === "thinking_delta") {
        app.statusLine.set("progress", getTheme().fg("muted", "Thinking..."));
        app.tui.requestRender();
        ...
        app.chatView.rebuildChat();
        app.tui.requestRender();
        app.extensionHost.dispatchEvent({
        ...
        app.statusLine.set(
          "progress",
          getTheme().fg(
            e.isError ? "error" : "success",
            `${n} ${e.isError ? "failed" : "completed"}`,
          ),
        );
        const tid = tcIds.get(e.id);
        if (tid) app.chatView.endToolCall(tid, e.result, e.isError);
        app.chatView.rebuildChat();
        app.tui.requestRender();
        app.extensionHost.dispatchEvent({
          type: "tool_call_end",
    ...
    app.cumulativeInput += u.input;
    app.cumulativeOutput += u.output;
    app.cumulativeCacheRead += u.cacheRead;
    app.cumulativeCacheWrite += u.cacheWrite;
    app.cumulativeCost += u.cost;
    app.chatView.rebuildFromTranscript(
      app.transcript,
      r.status === "max_steps"
    ...
    app.updateHeader();
    app.updateFooter();
    app.statusLine.set("progress", undefined);
    app.running = false;
    app.chatView.rebuildChat();
    app.tui.requestRender();
    app.extensionHost.dispatchEvent({
...
export function doSubmit(app: SubmitDeps, text: string): void {
  ...
  app.chatView.addMessage("user", expanded);
  app.chatView.rebuildChat();
  app.tui.requestRender();
