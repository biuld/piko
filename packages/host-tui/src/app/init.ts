...
export async function initApp(app: InitDeps, options: RunTuiOptions): Promise<void> {
  ...
  if (options.sessionName && !app.sessionName) { await app.host.setSessionName(options.sessionName); app.sessionName = options.sessionName; }

  app.host.onAfterRebind(async () => { await app.host.restoreFromSession(); await app.syncTranscript(); });
  ...
  app.editor.onSubmit = (text: string) => {
    ...
    if (app.running) {
      app.host.steer(t);
      app.chatView.addMessage("system", `Queued for next turn: ${t.slice(0, 80)}${t.length > 80 ? "..." : ""}`);
      app.chatView.rebuildChat(); app.tui.requestRender(); return;
    }
    const extCmd = app.extensionHost.findCommand(t);
    if (extCmd) { extCmd.handler(t.slice(extCmd.value.length).trim(), { theme: getTheme(), setEditorText: (s: string) => app.editor.setText(s), getEditorText: () => app.getEditorComponent().getText() } as any); return; }
  ...
  tui.addInputListener((data: string) => {
    ...
    if (data.includes("\x1b[201~")) {
      ...
      if (pb.length > 100 && isImageData(Buffer.from(pb, "binary"))) { void _imagePaste(app, Buffer.from(pb, "binary")); pb = ""; return { consume: true }; }
      pb = ""; return undefined;
    }
    if (pb.length > 0) { pb += data; return undefined; }
    if (data === "\u0010") { void app.cycleModel(false); return { consume: true }; }
    if (data === "\u000e") { void app.cycleModel(true); return { consume: true }; }
    return undefined;
  });

  app.terminal.setTitle("piko");
  process.on("SIGINT", () => {
    if (app.abortController && !app.abortController.signal.aborted) { app.abortController.abort(); app.spinner.stop(); app.statusLine.set("progress", getTheme().fg("error", "Interrupted")); }
    else if (!app.abortController) process.exit(0);
  });

  app.updateHeader();
  if (app.host.sessionFile) { app.chatView.rebuildFromTranscript(app.transcript); await app.resume(); }
  else { app.chatView.addMessage("system", "New session  |  Ctrl+D submit  Ctrl+C exit  /help"); app.updateFooter(); app.chatView.rebuildChat(); }
  tui.start();
}
