//! Conversation Map view-model from session tree entries.
...
pub struct ConversationMapViewModel {
...
pub fn prune_expansion(expanded: &mut HashSet<String>, surviving_ids: &HashSet<String>) {
    expanded.retain(|id| surviving_ids.contains(id));
}

/// Derive map for the selected agent. `preview_entry_id` / `expanded` are GUI-local.
pub fn derive_conversation_map(
    state: &ClientState,
    preview_entry_id: Option<&str>,
    expanded: &HashSet<String>,
) -> ConversationMapViewModel {
    let Some(session) = state.live_session.as_ref() else {
        return ConversationMapViewModel::default();
    };
    ...
    let visible: Vec<MapNode> = all_nodes
        .into_iter()
...
fn entry_label(entry: &SessionTreeEntry) -> (MapEntryKind, String) {
    match entry {
        SessionTreeEntry::Message(m) => {
            ...
            (MapEntryKind::Message, label)
        }
        SessionTreeEntry::ToolCall(t) => (MapEntryKind::Tool, format!("tool {}", t.tool_name)),
        SessionTreeEntry::ModelChange(m) => (MapEntryKind::System, format!("model {}", m.model_id)),
        SessionTreeEntry::ThinkingLevelChange(t) => (
            MapEntryKind::System,
            format!("thinking {}", t.thinking_level),
        ),
        SessionTreeEntry::BranchSummary(_) => (MapEntryKind::System, "branch summary".into()),
        SessionTreeEntry::Compaction(_) => (MapEntryKind::System, "compaction".into()),
        SessionTreeEntry::Label(l) => (
            MapEntryKind::Other,
            l.label.clone().unwrap_or_else(|| l.target_id.clone()),
        ),
        other => (MapEntryKind::Other, other.id().to_string()),
    }
}
...
mod tests {
    ...
    fn marks_active_path_and_off_path_preview() {
        ...
        let expanded = default_map_expansion(&entries, Some("b"));
        let vm = derive_conversation_map(&state, Some("c"), &expanded);
    ...
    fn on_path_preview_does_not_enable_switch() {
        ...
        let expanded = default_map_expansion(&entries, Some("b"));
        let vm = derive_conversation_map(&state, Some("a"), &expanded);
        assert!(!vm.can_switch_branch);
    }
    ...
    fn collapsed_parent_hides_children() {
        ...
        let vm = derive_conversation_map(&state, None, &expanded);
    ...
    fn prune_keeps_surviving_expansion_ids() {
        ...
        prune_expansion(&mut expanded, &surviving);
