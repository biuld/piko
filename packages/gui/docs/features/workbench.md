...
[GUI Overlay Stack](overlay-stack.md)); toasts and narrow-window
...
- Command Palette is a Transient overlay opened with `Cmd+Shift+P` (see
  [GUI Command Palette](command-palette.md)).
...
are covered by
[GUI Chrome Presentation](chrome-presentation.md). Map expansion,
