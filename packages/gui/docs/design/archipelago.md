...
A Archipelago is not only the center content. It owns the **full chrome
...
1. **Archipelago** = the exclusive **frame** below the window and above
...
6. Overlay stack stays orthogonal and paints above the active Archipelago.
...
```text
DesktopApp
├── ArchipelagoId / ArchipelagoRouter                    # exclusive frame (switched together)
│   ├── Workbench
...
| Layer | Role |
|---|---|
| Archipelago | Where the user *is* — full frame (bars + body) |
...
```text
enum ArchipelagoId / ArchipelagoRouter {
    Workbench,
...
- **v1:** instantaneous swap of the entire Archipelago frame.
...
```text
packages/gui/src/
├── app/archipelago.rs     # ArchipelagoId / ArchipelagoRouter enum (composition root)
...
| `app/desktop_app.rs` | Switch whole frame via `ArchipelagoId / ArchipelagoRouter`; no global bars |

### 8.3 Naming rule

- **Archipelago** — the active full frame (bars + body).
...
- Stacked Archipelagos
- Shared global TitleBar/StatusBar with Minimal/Hidden flags
- Embedding Settings in the Workbench center column
- Replacing Overlay with Archipelago for Palette / Approvals
- Animating the first implementation
- Migrating TUI onto Archipelago concepts

## 11. Implementation sequence

1. Introduce `ArchipelagoId / ArchipelagoRouter` + `match` that renders a full Workbench frame
