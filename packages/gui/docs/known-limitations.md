...
- **No durable Session ownership.** Transcripts and tree facts come from hostd
  reconcile; GUI keeps drafts, follow, and map expansion as presentation state.
...
- **Layout persistence scope.** Split sizes, pane visibility, and reduced-motion
  preference persist through hostd's `[gui]` settings. Focus, map expansion,
  drafts, and per-Agent scroll/follow positions remain window-local.
