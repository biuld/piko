...
> Parent: [GPUI Desktop Client Design](gui-design.md)
> Related: [GUI Overlay Stack Design](gui-overlay-stack-design.md),
...
This design also reorganizes `packages/gui/src/chrome` so each surface is a
...
- `chrome/` is structured as if Workbench *is* chrome.
...
**Settings-owned** UI under `chrome/settings/`, not a Workbench StatusBar mode.
...
```text
packages/gui/src/chrome/
├── mod.rs
├── primary_surface.rs     # enum, navigate open/close, focus save/restore
├── island/                # IslandPanel shell (Workbench building block)
├── overlay/               # above all surfaces
├── widgets/               # shared primitives (brand label, ghost icon btn, …)
├── workbench/             # PrimarySurface::Workbench (full frame)
│   ├── mod.rs             # render_frame() → title + body + status
│   ├── title_bar.rs       # dock toggles + gear
│   ├── status_bar.rs      # moved from chrome/status_bar.rs
│   ├── assemble.rs        # island columns (body)
│   ├── center.rs
│   ├── left.rs
│   ├── right.rs
│   └── layout/
└── settings/              # PrimarySurface::Settings (full frame)
    ├── mod.rs             # render_frame() → title + body (+ no status v1)
    ├── title_bar.rs       # Back + title
    ├── body.rs            # nav + panel host
    ├── nav.rs
    └── sections/
```
...
match primary_surface {
  Workbench => chrome::workbench::render_frame(...),
  Settings  => chrome::settings::render_frame(...),
}
...
| `chrome/overlay/*` | Unchanged; still window-level |
