...
- The platform policy requires the model to use exact names from the
  structured tool surface when describing or invoking tools. It must not
  invent aliases or claim tools that are absent from that surface.
...
11. A user asks which tools are available. The model reports the exact names
    supplied for the run (for example `web_search`), without shortening names
    (for example to `search`) or inventing related tools such as page readers.
...
- [x] **Superseded (F-04 slice 2)**: the run facts are emitted as a retained
...
- [x] A hostd snapshot with host facts emits exactly one `environment.host`
      block with the captured facts in the fixed key order, and omits
      unavailable facts.
- [x] A hostd snapshot with no host facts emits no `environment.host` block.
- [x] A run whose previous model differs from the current model emits exactly
      one `context.model-switch` block naming both models; first runs and
      unchanged-model runs emit none.
- [x] The RunDynamic blocks (`environment.host`, `context.model-switch`)
      never change
      `semantic_prefix_digest`; a change in project context still does
      (regression guard).
- [x] Identical inputs produce identical `environment.host` and
      `context.model-switch` block content and digests (determinism).
- [x] The world-state full-snapshot content marks `run_kind: continuation`
...
- [x] The platform policy tells the model to use exact structured-tool names
      for both descriptions and calls, and forbids invented aliases or absent
      tools.
