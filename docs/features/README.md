...
| F-15 | observability | End-to-end tracing (turn → agent → model step → tool → child agent), OTLP HTTP export + metrics, per-turn usage accounting (hostd ledger + OTel projection), rollout, turn timing | implemented (F-15/D-15/V-15 + D-29/V-29) | core/src/otel_init.rs, rollout*.rs, turn_timing.rs |
