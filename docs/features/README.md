...
| F-52 | session-history-inspector | Read-only TUI trajectory view organized from authoritative journal facts: one flat per-agent time stream spanning works, a two-lane timing strip (Model/Tools) with a shared time axis, and tabbed detail (Summary/Payload/Result/Timing) enriched by diagnostic trajectory joins | redesign implemented (D-69/ADR-028/ADR-029); projection, paging, restart E2E, and web-viewer retirement landed; single-view TUI rebuild landed; E2E assertions updated | piko product decision; F-31/F-36/F-37/F-48/F-51 |
